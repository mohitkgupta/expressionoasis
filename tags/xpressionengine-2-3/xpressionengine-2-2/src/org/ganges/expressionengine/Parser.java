 ...
 * 
 * <br/><br/>Some examples of expression <hr/><br/>1). <i>123 + 124 - 67 * 45
 * div 90 + (12 + 34 div 67) </i>. <br/>2). <i>principal * pow(( 1 - rate div
 * 100 ), time) </i>. <br/>3). <i>120 * pow(sin(20) div tan(30), 2) </i>.
 * <br/>4). <i>values[0] + values[1] + values[2] + 120 * 45 div num[1][1] </i>.
 * <br/>5). <i>person.address.city.name + students[0].rollNo </i>. <br/>6). <i>
 * /person/address/city/name + /person/children[name='Ram'] +
 * /person/children[1] </i>. <br/>7). <i>abc=xyz || 1!=2 && 3 >= xyz && xvz <=
 * 30 </i>. <br/><br/>
 * 
 * @author Mohit Gupta
...
public final class Parser {
	...
	private void parse( String expression, List<ExpressionToken> tokenList, int currentPosition, int offset )
		...
		LOGGER.debug( "next-token[" + currentExpressionCharacter + "] currentToken[" + getCurrentToken( expression, offset, currentPosition )
				+ "] + lastToken[" + getLastToken( tokenList ) + "]" );
		...
		// Then we check if last token was a delimiter and current delimiter can be added to it to form a valid dilimiter
		...
		if( grammar.isDelimiter( currentExpressionCharacter )
				&& !grammar.isApproachable( getCurrentToken( expression, offset, currentPosition ) + currentExpressionCharacter ) ) {
		...
		// collected as single token
				...
				&& grammar.isApproachable( getLastToken( tokenList ) + getCurrentToken( expression, offset, currentPosition ) ) ) {
